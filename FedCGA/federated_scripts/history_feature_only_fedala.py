...
class HistoryFeatureALA:
    def __init__(self, cid, loss, train_data, batch_size, rand_percent,
                 layer_idx=0, eta=1.0, device='cpu', threshold=0.1,
                 num_pre_loss=10, history_feature_init=True,
                 history_features=None, init_strategy='adaptive'):
        self.cid = cid
        self.loss = loss
        ...
        self.history_feature_init = history_feature_init
        self.history_features = history_features or {}
        self.init_strategy = init_strategy

    def _compute_base_weight_by_history_features(self, history_features):
        base_weight = 1.0

        if 'client_trust' in history_features:
            trust_score = history_features.get('client_trust', 0.5)
            base_weight *= (0.3 + 0.7 * trust_score)

        if 'stability_score' in history_features:
            stability = history_features.get('stability_score', 0.5)
            base_weight *= (0.5 + 0.5 * stability)

        return min(max(base_weight, 0.0), 2.0)

    def _adjust_eta_by_history_features(self, gradient, history_features):
        base_eta = self.eta
        eta_adjust = 1.0

        if 'client_trust' in history_features:
            trust_score = history_features.get('client_trust', 0.5)
            eta_adjust *= (0.8 + 0.4 * trust_score)

        if 'stability_score' in history_features:
            stability = history_features.get('stability_score', 0.5)
            eta_adjust *= (0.7 + 0.6 * stability)

        return base_eta * max(0.1, min(eta_adjust, 2.0))

    def _initialize_weights_by_history_features(self, params_p, history_features):
        if not self.history_feature_init or not history_features:
            return [torch.ones_like(param.data).to(self.device) for param in params_p]

        initial_weights = []

        for i, param in enumerate(params_p):
            base_weight = self._compute_base_weight_by_history_features(history_features)
    ...
    def adaptive_local_aggregation(self, global_model, local_model):
        ...
        if self.weights is None:
            self.weights = self._initialize_weights_by_history_features(params_p, self.history_features)
            ...
            for x, y in rand_loader:
                ...
                for param_t, param, param_g, weight in zip(params_tp, params_p, params_gp, self.weights):
                    if param_t.grad is not None:
                        ...
                        history_aware_eta = self._adjust_eta_by_history_features(grad_signal, self.history_features)
                        weight_update = weight - history_aware_eta * grad_signal
...
class HistoryFeatureFedALATrainingSystem:
    ...
    def __init__(self, config_path=None):
        ...
        self.history_feature_config = self.config.get('history_feature_config', {})
        self.history_feature_enabled = self.history_feature_config.get('enabled', True)
        self.init_strategy = self.history_feature_config.get('init_strategy', 'adaptive')

        self.client_history_records = {}

        print("✅ FedALA训练系统初始化完成（历史特征感知）")

    def _resolve_config_path(self, config_path):
        if config_path is None:
            config_path = os.path.join(self.project_root, 'configs', 'history_feature_fedala.yaml')
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists(backup_config):
                print(f"⚠️ 历史特征配置不存在，使用FedALA配置: {backup_config}")
    ...
    def _setup_output_directories(self):
        ...
        algorithm = 'fedala_history_feature'
    ...
    def _update_client_history(self, client_id, train_loss, compute_time):
        if client_id not in self.client_history_records:
            self.client_history_records[client_id] = {
                'loss_history': [],
                'time_history': [],
                'update_count': 0
            }

        self.client_history_records[client_id]['loss_history'].append(train_loss)
        self.client_history_records[client_id]['time_history'].append(compute_time)
        self.client_history_records[client_id]['update_count'] += 1

        max_history = self.history_feature_config.get('max_history_length', 10)
        if len(self.client_history_records[client_id]['loss_history']) > max_history:
            self.client_history_records[client_id]['loss_history'].pop(0)
            self.client_history_records[client_id]['time_history'].pop(0)

    def _get_client_history_features(self, client_id, round_num, total_rounds):
        if not self.history_feature_enabled:
            return {}

        history_features = {}

        if client_id in self.client_history_records:
            record = self.client_history_records[client_id]

            if record['loss_history']:
                avg_loss = np.mean(record['loss_history'])
                client_trust = max(0.1, 1.0 - min(avg_loss / 10.0, 0.9))
                history_features['client_trust'] = client_trust

                if len(record['loss_history']) >= 2:
                    loss_std = np.std(record['loss_history'])
                    stability_score = max(0.1, 1.0 - min(loss_std, 0.9))
                    history_features['stability_score'] = stability_score
                else:
                    history_features['stability_score'] = 0.5
            else:
                history_features['client_trust'] = 0.7
                history_features['stability_score'] = 0.6
        else:
            history_features['client_trust'] = 0.7
            history_features['stability_score'] = 0.6
    ...
    def client_local_train(self, global_model, client_id, local_epochs=None, round_num=0, total_rounds=0):
        ...
        history_features = self._get_client_history_features(client_id, round_num, total_rounds)

        ala = HistoryFeatureALA(
            cid=client_id,
            loss=nn.MSELoss(),
            train_data=dataset,
            ...
            history_feature_init=self.history_feature_enabled,
        ...
        try:
            ...
            print(f"⚠️ 历史特征感知ALA执行失败: {e}, 使用标准训练")
        ...
        self._update_client_history(client_id, train_loss, client_time)
    ...
    def _print_training_config(self):
        ...
        print(f"   历史特征感知: {'启用' if self.history_feature_enabled else '禁用'}")
        if self.history_feature_enabled:
    ...
    def run_training(self):
        ...
        final_result = {
            ...
            'history_feature_configuration': {
                'history_feature_enabled': self.history_feature_enabled,
                'init_strategy': self.init_strategy
            },
            'client_history_summary': {
                'total_clients_with_history': len(self.client_history_records),
                'avg_updates_per_client': np.mean([r['update_count'] for r in
                                                   self.client_history_records.values()]) if self.client_history_records else 0
            },
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='FedALA联邦学习训练脚本（历史特征感知）')
    parser.add_argument('--config', type=str, help='配置文件路径（可选）')
    args = parser.parse_args()

    try:
        trainer = HistoryFeatureFedALATrainingSystem(args.config)
