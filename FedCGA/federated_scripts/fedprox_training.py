...
class FedProxTrainingSystem:
    ...
    def __init__(self, config_path=None):
        ...
        print("✅✅ FedProx训练系统初始化完成（设备分离模式）")

    def _resolve_config_path(self, config_path):
        if config_path is None:
            config_path = os.path.join(self.project_root, 'configs', 'fedprox_config.yaml')
    ...
    def client_local_train(self, global_model, client_id, local_epochs=None):
        ...
        global_params = {k: v.detach().clone() for k, v in client_model.named_parameters()}
    ...
    def fedprox_aggregate(self, client_updates):
    ...
    def _finalize_training(self, model, history, test_clients, initial_metrics,
        ...
        final_result = {
            ...
            'algorithm_info': {
                'algorithm': 'fedprox',
                'proximal_mu': self.config.get('mu', 0.01)
            },
    ...
    def _print_final_summary(self, result, compute_stats):
        ...
        mu = result['algorithm_info']['proximal_mu']
        print(f"🔧🔧🔧🔧 FedProx配置:")
        print(f"   近端项系数 μ: {mu}")
    ...
    def run_training(self):
        ...
        for round_num in range(rounds):
            ...
            if client_updates:
                new_global_params = self.fedprox_aggregate(client_updates)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='FedProx联邦学习训练脚本（设备分离版）')
    parser.add_argument('--config', type=str, help='配置文件路径（可选）')

    args = parser.parse_args()

    try:
        trainer = FedProxTrainingSystem(args.config)
