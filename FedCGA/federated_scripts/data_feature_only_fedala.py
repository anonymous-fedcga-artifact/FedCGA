...
class DataFeatureALA:
    def __init__(self, cid, loss, train_data, batch_size, rand_percent,
        ...
        self.data_feature_init = data_feature_init
        self.data_features = data_features or {}
        self.init_strategy = init_strategy

    def _compute_base_weight_by_data_features(self, data_features):
        base_weight = 1.0

        if 'sample_count' in data_features and data_features['sample_count'] > 0:
            sample_count = data_features['sample_count']
            sample_weight = min(1.0, np.log(sample_count + 1) / 5.0)
            base_weight *= (0.7 + 0.3 * sample_weight)

        if 'data_quality' in data_features:
            quality_score = data_features.get('data_quality', 0.5)
            base_weight *= (0.5 + quality_score)

        return min(max(base_weight, 0.0), 2.0)

    def _adjust_eta_by_data_features(self, gradient, data_features):
        base_eta = self.eta
        eta_adjust = 1.0

        if 'sample_count' in data_features:
            sample_count = data_features.get('sample_count', 100)
            if sample_count < 50:
                eta_adjust *= 0.8
    ...
    def _initialize_weights_by_data_features(self, params_p, data_features):
        if not self.data_feature_init or not data_features:
            return [torch.ones_like(param.data).to(self.device) for param in params_p]

        initial_weights = []

        for i, param in enumerate(params_p):
            base_weight = self._compute_base_weight_by_data_features(data_features)
    ...
    def adaptive_local_aggregation(self, global_model, local_model):
        ...
        if self.weights is None:
            self.weights = self._initialize_weights_by_data_features(params_p, self.data_features)
            ...
            for x, y in rand_loader:
                ...
                for param_t, param, param_g, weight in zip(params_tp, params_p, params_gp, self.weights):
                    if param_t.grad is not None:
                        ...
                        data_aware_eta = self._adjust_eta_by_data_features(grad_signal, self.data_features)
                        weight_update = weight - data_aware_eta * grad_signal
...
class DataFeatureFedALATrainingSystem:
    ...
    def __init__(self, config_path=None):
        ...
        self.data_feature_config = self.config.get('data_feature_config', {})
        self.data_feature_enabled = self.data_feature_config.get('enabled', True)
        self.init_strategy = self.data_feature_config.get('init_strategy', 'adaptive')

        print("✅ FedALA训练系统初始化完成（数据特征感知）")

    def _resolve_config_path(self, config_path):
        if config_path is None:
            config_path = os.path.join(self.project_root, 'configs', 'data_feature_fedala.yaml')
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists(backup_config):
                print(f"⚠️ 数据特征配置不存在，使用FedALA配置: {backup_config}")
    ...
    def _get_client_data_features(self, client_id, round_num, total_rounds):
        if not self.data_feature_enabled:
            return {}
        ...
        if X is not None:
            data_features['sample_count'] = len(X)
            ...
            if sample_count < 10:
                ...
                data_quality = 0.9
            data_features['data_quality'] = data_quality
    ...
    def client_local_train(self, global_model, client_id, local_epochs=None, round_num=0, total_rounds=0):
        ...
        data_features = self._get_client_data_features(client_id, round_num, total_rounds)
        ...
        ala = DataFeatureALA(
            ...
            data_feature_init=self.data_feature_enabled,
        ...
        try:
            ...
            print(f"⚠️ 数据特征感知ALA执行失败: {e}, 使用标准训练")
    ...
    def _print_training_config(self):
        ...
        print(f"   数据特征感知: {'启用' if self.data_feature_enabled else '禁用'}")
        if self.data_feature_enabled:
    ...
    def run_training(self):
        ...
        final_result = {
            ...
            'data_feature_configuration': {
                'data_feature_enabled': self.data_feature_enabled,
                'init_strategy': self.init_strategy
            },
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='FedALA联邦学习训练脚本（数据特征感知）')
    parser.add_argument('--config', type=str, help='配置文件路径（可选）')
    args = parser.parse_args()

    try:
        trainer = DataFeatureFedALATrainingSystem(args.config)
