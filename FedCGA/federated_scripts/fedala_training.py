...
class ALA:
    def __init__(self, cid, loss, train_data, batch_size, rand_percent,
                 layer_idx=0, eta=1.0, device='cpu', threshold=0.1, num_pre_loss=10):
        self.cid = cid
        self.loss = loss
            ...
            for x, y in rand_loader:
                ...
                for param_t, param, param_g, weight in zip(params_tp, params_p, params_gp, self.weights):
                    if param_t.grad is not None:
                        ...
                        weight.data = torch.clamp(weight - self.eta * grad_signal, 0, 1)
...
class FedALATrainingSystem:
    ...
    def __init__(self, config_path=None):
        ...
        print("✅ FedALA训练系统初始化完成")

    def _resolve_config_path(self, config_path):
        if config_path is None:
            config_path = os.path.join(self.project_root, 'configs', 'fedala_config.yaml')
        elif not os.path.isabs(config_path):
            config_path = os.path.join(self.project_root, config_path)

        if not os.path.exists(config_path):
            backup_config = os.path.join(self.project_root, 'configs', 'fedprox_config.yaml')
            if os.path.exists(backup_config):
                print(f"⚠️ FedALA配置不存在，使用FedProx配置: {backup_config}")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='FedALA联邦学习训练脚本')
    parser.add_argument('--config', type=str, help='配置文件路径（可选）')
    args = parser.parse_args()

    try:
        trainer = FedALATrainingSystem(args.config)
