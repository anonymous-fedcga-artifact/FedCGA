...
class StateFeatureALA:
    def __init__(self, cid, loss, train_data, batch_size, rand_percent,
        ...
        self.state_feature_init = state_feature_init
        self.state_features = state_features or {}
        self.init_strategy = init_strategy

    def _compute_base_weight_by_state_features(self, state_features):
        base_weight = 1.0

        if 'training_progress' in state_features:
            progress = state_features.get('training_progress', 0.0)
            if progress < 0.3:
                base_weight *= 1.3
            elif progress > 0.7:
                base_weight *= 0.7
        ...
        if 'overfit_risk' in state_features:
            overfit_risk = state_features.get('overfit_risk', 0.0)
            if overfit_risk > 0.5:
                base_weight *= 0.6
            elif overfit_risk > 0.3:
                base_weight *= 0.8
            else:
                base_weight *= 1.0
    ...
    def _adjust_eta_by_state_features(self, gradient, state_features):
        base_eta = self.eta
        eta_adjust = 1.0
    ...
    def _initialize_weights_by_state_features(self, params_p, state_features):
        if not self.state_feature_init or not state_features:
            return [torch.ones_like(param.data).to(self.device) for param in params_p]

        initial_weights = []

        for i, param in enumerate(params_p):
            base_weight = self._compute_base_weight_by_state_features(state_features)
    ...
    def adaptive_local_aggregation(self, global_model, local_model):
        ...
        if self.weights is None:
            self.weights = self._initialize_weights_by_state_features(params_p, self.state_features)
            ...
            for x, y in rand_loader:
                ...
                for param_t, param, param_g, weight in zip(params_tp, params_p, params_gp, self.weights):
                    if param_t.grad is not None:
                        ...
                        state_aware_eta = self._adjust_eta_by_state_features(grad_signal, self.state_features)
                        weight_update = weight - state_aware_eta * grad_signal
...
class StateFeatureFedALATrainingSystem:
    ...
    def __init__(self, config_path=None):
        ...
        self.state_feature_config = self.config.get('state_feature_config', {})
        self.state_feature_enabled = self.state_feature_config.get('enabled', True)
        self.init_strategy = self.state_feature_config.get('init_strategy', 'adaptive')

        self.previous_val_losses = []
        self.overfit_monitor_window = 5

        print("✅ FedALA训练系统初始化完成（状态特征感知）")

    def _resolve_config_path(self, config_path):
        if config_path is None:
            config_path = os.path.join(self.project_root, 'configs', 'state_feature_fedala.yaml')
        ...
        if not os.path.exists(config_path):
            ...
            if os.path.exists(backup_config):
                print(f"⚠️ 状态特征配置不存在，使用FedALA配置: {backup_config}")
    ...
    def _setup_output_directories(self):
        ...
        algorithm = 'fedala_state_feature'
    ...
    def _calculate_overfit_risk(self, val_metrics):
        if not val_metrics or 'avg_rmse' not in val_metrics:
            return 0.0

        current_val_loss = val_metrics['avg_rmse']
        self.previous_val_losses.append(current_val_loss)

        if len(self.previous_val_losses) > self.overfit_monitor_window:
            self.previous_val_losses.pop(0)

        if len(self.previous_val_losses) < 3:
            return 0.0

        recent_losses = self.previous_val_losses[-3:]
        if len(recent_losses) >= 3:
            loss_increase = False
            for i in range(1, len(recent_losses)):
                if recent_losses[i] > recent_losses[i - 1] * 1.05:
                    loss_increase = True
                    break

            if loss_increase:
                return 0.8
            elif recent_losses[-1] > recent_losses[0] * 1.02:
                return 0.5
            else:
                return 0.2
        else:
    ...
    def _get_client_state_features(self, client_id, round_num, total_rounds, val_metrics=None):
        if not self.state_feature_enabled:
            return {}

        state_features = {}

        if total_rounds > 0:
            training_progress = round_num / total_rounds
            state_features['training_progress'] = min(max(training_progress, 0.0), 1.0)
        else:
            state_features['training_progress'] = 0.0

        overfit_risk = self._calculate_overfit_risk(val_metrics)
        state_features['overfit_risk'] = overfit_risk
    ...
    def client_local_train(self, global_model, client_id, local_epochs=None, round_num=0, total_rounds=0,
        ...
        state_features = self._get_client_state_features(client_id, round_num, total_rounds, val_metrics)
        ...
        ala = StateFeatureALA(
            ...
            state_feature_init=self.state_feature_enabled,
        ...
        try:
            ...
            print(f"⚠️ 状态特征感知ALA执行失败: {e}, 使用标准训练")
    ...
    def _print_training_config(self):
        ...
        print(f"   状态特征感知: {'启用' if self.state_feature_enabled else '禁用'}")
        if self.state_feature_enabled:
    ...
    def run_training(self):
        ...
        for round_num in range(rounds):
            ...
            if round_num > 0:
                val_metrics_before = self.evaluate_model(global_model, val_clients, 'val')
            else:
                val_metrics_before = None
            ...
            for i, client_id in enumerate(selected_clients):
                try:
                    update = self.client_local_train(
                        ...
                        round_num=round_num,
                        total_rounds=rounds,
                        val_metrics=val_metrics_before
                    )
                    if update:
            ...
            if client_updates:
                ...
                if (round_num % 10 == 0) or (round_num == rounds - 1):
                    ...
                    if val_metrics:
                        ...
                        state_features = self._get_client_state_features(
                            "server", round_num, rounds, val_metrics
                        )
                        print(f"   训练进度: {state_features.get('training_progress', 0.0):.2%}")
                        print(f"   过拟合风险: {state_features.get('overfit_risk', 0.0):.2%}")
        ...
        final_result = {
            ...
            'state_feature_configuration': {
                'state_feature_enabled': self.state_feature_enabled,
                'init_strategy': self.init_strategy
            },
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='FedALA联邦学习训练脚本（状态特征感知）')
    parser.add_argument('--config', type=str, help='配置文件路径（可选）')
    args = parser.parse_args()

    try:
        trainer = StateFeatureFedALATrainingSystem(args.config)
