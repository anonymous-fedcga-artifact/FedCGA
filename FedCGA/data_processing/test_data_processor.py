...
class TestDataProcessor(TrainDataProcessor):
    def __init__(self, config_path="../configs/data_config.yaml"):
        ...
        self.output_dir = os.path.join(project_root, "data", "processed", "test_features")
    ...
    def load_data(self, filename="test_set.csv"):
    ...
    def prepare_test_driver_data(self, driver_data):
        available_cols = [
    ...
    def process_drivers(self, df):
        ...
        for driver_id in tqdm(df[car_id_col].unique(), desc="Test drivers"):
            driver_data = df[df[car_id_col] == driver_id].copy()

            if len(driver_data) < min_samples:
                continue

            X, y, feature_names = self.prepare_test_driver_data(driver_data)
    ...
    def save_results(self, results, metadata):
        ...
        metadata_path = os.path.join(self.metadata_dir, "test_metadata.csv")
    ...
    def run(self, filename="test_set.csv"):
        ...
        if not results:
            raise RuntimeError("No valid test driver data was generated.")

        info = self.save_results(results, metadata)

        print(
            f"Processed {info['total_drivers_processed']} test drivers, "
